...
### 利用する機能
1. パフォーマンス・ハブ（SQLモニタリング、ASH分析）でパフォーマンスが悪い以下のSQLを探索、SQLモニタリングの画面で実行計画がTable Storage Scanになっていることを確認します。
...
4.ジョブの実行が完了したら、SQLを再度実行します。パフォーマンス・ハブから実行したSQLの実行計画が変わっていることを確認してください。
